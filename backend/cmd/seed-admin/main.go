...
func main() {
	...
	for _, connector := range defaultConnectors {
		_, err = tx.Exec(ctx,
			...
			uuid.New(), orgID, connector.Name, connector.Type, connector.URL, true, "none")
		if err != nil {
			log.Fatalf("Failed to create default connector '%s': %v", connector.Name, err)
		}
	}
